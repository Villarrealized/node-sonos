 ...
 * Get Music Library Information
 * @param  {String}   searchType  Choice - artists, albumArtists, albums, genres, composers, tracks, playlists, share
 * @param  {String}   searchTerms  Optional - search term to search for
 * @param  {Object}   requestOptions     Optional - default {start: 0, total: 100}
 * @param  {String}   separator   Optional - default is colon. `:` or `/`
 * @param  {Number}   browseAllLevel   Optional - How many levels deep to browse on the 'All' item
 *                                     Ex. browseLevel = 0 'A:GENRE/Jazz' -> returns all artists in the Jazz Genre
 *                                     Ex. browseLevel = 1 'A:GENRE/Jazz/' -> returns all albums in the Jazz Genre
 *                                     Ex. browseLevel = 2 'A:GENRE/Jazz//' -> returns all songs in the Jazz Genre
 * @returns {Object} {returned: {String}, total: {String}, items:[{title:{String}, uri: {String}}]}
 */
Sonos.prototype.searchMusicLibrary = async function (searchType, searchTerms, requestOptions = {}, separator = ':', browseAllLevel = 0) {
  const searchTypes = {
    'artists': 'A:ARTIST',
  ...
  if (searchTerms && !Array.isArray(searchTerms)) searchTerms = [searchTerms]

  if (searchTerms && searchTerms.length) {
    let joinedTerms = searchTerms.map(term => encodeURIComponent(term)).join('/')
    // don't want to encode the share searchTerms, otherwise it won't work
    if (searchType === 'share') {
      joinedTerms = searchTerms.join('/')
    }
    const trailingSlashes = '/'.repeat(browseAllLevel)
    searches = searches.concat(joinedTerms, trailingSlashes)
  }
...
Sonos.prototype.setPlayMode = async function (playmode) {
  ...
  if (playmodes.indexOf(playmode) === -1) return new Error('Invalid play mode: ' + playmode)
